...
5. Load the relevant convention files from `.ai/conventions/react-tauri/` for the touched surface: windowing, IPC/permissions, state, accessibility, performance, or cross-platform.
...
3. **Write tests first (Red)** — Before writing production code, write unit tests for all non-trivial logic: front-end (Vitest + RTL; follow `.ai/skills/testing-pro/references/frontend.md`) and/or Rust (cargo-nextest + tokio + mockall; follow `.ai/skills/testing-pro/references/rust.md`). Cover happy path and at least one failure path per non-trivial behavior. Confirm tests compile and **fail** at this point.
