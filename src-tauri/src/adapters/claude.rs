...
/// Parse `stdout` as a single JSON document. Valid JSON is parsed as-is; only
/// when that fails do we strip ANSI escapes defensively and retry (§5), so a
/// `result` payload that legitimately contains escape-like bytes is preserved.
fn parse_json_lenient(stdout: &str) -> Result<Value, AdapterError> {
    let trimmed = stdout.trim();
    if trimmed.is_empty() {
        return Err(AdapterError::OutputParseFailure {
            detail: "claude produced no output".to_string(),
        });
    }
    ...
    serde_json::from_str::<Value>(stripped.trim()).map_err(|e| AdapterError::OutputParseFailure {
        detail: format!("claude output was not valid JSON: {e}"),
    })
}
