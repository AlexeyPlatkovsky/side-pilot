...
| Enabled providers | `useState` in `ChatPanel`, loaded from `getCliIntegrations()` | `AssistantId[] \| null` (filters AiSwitcher; null = loading, empty = all disabled → composer hidden) |
